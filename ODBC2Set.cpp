...
// ODBC에서는 자동으로 열 값을 요청된 형식으로 변환하려고 합니다
	RFX_Text(pFX, _T("[ST_VEHICLE]"), m_ST_CAR);
	RFX_Text(pFX, _T("[ST_NAME]"), m_ST_NAME);
	RFX_Bool(pFX, _T("[ST_ELECTRIC]"), m_ST_ELEC);
	RFX_Text(pFX, _T("[ST_PHONE]"), m_ST_PHONE);
	RFX_Date(pFX, _T("[ST_INTIME]"), m_ST_INTIME);

	/*RFX_Text(pFX, _T("[테이블2].[ST_DE_VEHICL]"), m_ST_DE_VEHICL);
	RFX_Text(pFX, _T("[테이블2].[ST_DE_NAME]"), m_ST_DE_NAME);
	RFX_Text(pFX, _T("[테이블2].[ST_DE_PHONE]"), m_ST_DE_PHONE);
	RFX_Date(pFX, _T("[테이블2].[ST_DE_INTIME]"), m_ST_DE_INTIME);
	RFX_Date(pFX, _T("[테이블2].[ST_DE_OUTTIME]"), m_ST_DE_OUTTIME);
	RFX_Int(pFX, _T("[테이블2].[ST_DE_COST]"), m_ST_DE_COST);*/
}
/////////////////////////////////////////////////////////////////////////////
// CODBC2Set 진단
